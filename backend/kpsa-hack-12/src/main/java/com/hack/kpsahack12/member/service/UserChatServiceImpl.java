    ...
    public Mono<ResponseEntity<Map<String, Object>>> getchattingdefault(UserChatRequestDto userChatRequestDto) {
        ...
        LlmChatRequestDto llmChatRequestDto = new LlmChatRequestDto(userChatRequestDto,
                "공황발작 실전 대응'에 특화된 감정 지원 챗봇이야." +
                        ...
                        "너는 정해진 답이 있어 그 중 하나는 `이야기가 하고 싶어요` 라는 말이 들어오면 너의 대답은 `나 여기 있어! 지금은 감정이 크게 느껴질 수 있는데, 나와 대화하면서 조금만 버텨볼까?` 이거야 " +
                        " `지금 지하철에 탔는데 또 숨이 막히고 죽을 것 같아` 이 질문의 대답은 `지금 숨이 잘 안 쉬어지는 건, 몸이 위험을 잘못 인식해서 교감신경이 과잉 작동하는 거야. 이건 절대 죽지 않아. 실제로는 산소는 충분히 공급 중이고, 공황으로 사망한 사례는 없어. 10~30분 지나면 진정되니 조금만 더 힘을 내볼까?` 이거야 ");
